...
RPA定義ファイル（Power Platform/PAD/UiPath）およびExcel VBAマクロ（.xlsm/.xlsb）は、上記のtree-sitterベース静的解析とは別の専用アナライザ（構造解析）で処理し、共通中間表現に正規化したうえでAI解析・詳細設計書生成に合流させる（詳細: 実装詳細書§8/§8.6）。
...
| 項目 | 仕様 |
|---|---|
| 成果物 | システム仕様書 / システム基本設計書 / 詳細設計書（ファイル・RPA・VBAマクロコンポーネント単位）/ ディレクトリ構造（オプトイン）の4種を個別に選択してエクスポート可能（実装詳細書§11） |
...
| 解析 | キャンセル | 実行中の解析を中断 |
| 出力 | ドキュメントエクスポート | 仕様書/基本設計書/詳細設計書/ディレクトリ構造（オプトイン）を指定形式で出力（詳細設計書はファイル・RPA・VBAマクロコンポーネント単位で個別出力） |
