...
| 解析対象 | ソースコード（テキスト形式のプログラムファイル）+ **RPA定義ファイル（Power Platform）** + **Excel VBAマクロ（.xlsm/.xlsb）**。コンパイル済みバイナリ（exe/dll等）は対象外。レガシー.xls・パスワード保護VBAプロジェクトはv1では対象外（§8.6、§10） |
...
- **サイドカー実体**: `newtonx_bridge.py` を PyInstaller で単一 exe 化し、Tauri の sidecar として同梱（対象はWindowsのみのため配布が単純）。ソースは `src-tauri/sidecar/`（`newtonx_bridge.py` + RPCメソッド名等の定数を集約した `constants.py`）に置く。**開発時点では PyInstaller化・Tauri sidecar登録は未実施**で、Rust側から `<config.ai.pythonExe> src-tauri/sidecar/newtonx_bridge.py <config_file_path>` を子プロセスとして直接起動する暫定実装（`infra/ai_client/bridge.rs`）。配布方式確定後（§10 #4）にPyInstaller化・sidecar登録へ切り替える
  - **`python_exe` はPATH解決の `python` 固定文字列にしない**（実機で発生した重大な不具合）: Explorerから起動したアプリの子プロセスとして `python` を素のコマンド名で起動すると、環境によっては開発venvではなく Windows のアプリ実行エイリアス（`%LOCALAPPDATA%\Microsoft\WindowsApps\python.exe`）に解決される。このスタブは非対話（パイプ入出力）環境では何も出力せず応答不能のまま固まり、`session.open`等あらゆるRPCが `config.ai.timeoutSecs` いっぱいまで無言でタイムアウトする（stderrにも一切ログが出ないため発見が難しい）。対策として `config.ai.pythonExe` を設け、既定値はプロジェクト同梱の `myvenv/Scripts/python.exe` を絶対パスで指すようにした（ゼロハードコーディング原則。設定画面から変更可能）
...
```json
{
  ...
  "ai": {
    ...
    "outputLanguage": "ja",
    "pythonExe": "<プロジェクトルート>/myvenv/Scripts/python.exe"
  },
  "cache": {
    "enabled": true,
    "ttlDays": 30,
...
- `pythonExe` の既定値は `Default` 実装内で `CARGO_MANIFEST_DIR`（`src-tauri/`）基準に動的算出する（開発時専用。配布時はPyInstaller化サイドカーに置き換わるため無関係になる。§10 #4）。PATH解決に依存する `"python"` 固定値は前述のWindowsアプリ実行エイリアス問題により使用禁止
- `outputDir` 空文字時は「プロジェクトフォルダ/projectlens-docs/」を既定とする（この既定パス文字列も constants に定義）。配下のディレクトリ構成は §11 参照
- `scan.vba.maxFileSizeKb: 10240` は、Excelワークブックがコードファイルよりサイズが大きくなりやすいことを踏まえた既定値（§8.6）。非コードファイルの足切り判定自体は `max(scan.rpa.maxFileSizeKb, scan.vba.maxFileSizeKb)` を用いる（scanner.rs）
...
### 8.6 VBAマクロ解析設計（Excel）

FR-13。RPA解析（§8）と同様に、既存の4フェーズの枠組みをそのまま流用し、静的解析のみ専用アナライザに差し替える。

#### 8.6.1 対応形式と検出方式

| 対象 | 形式 | 検出方式 |
|---|---|---|
| Excel VBAマクロ | `.xlsm` / `.xlsb`（OOXML zipコンテナ内に `xl/vbaProject.bin`〈CFB/OLE2〉を含む） | 拡張子（config.scan.vba.extensions）+ zip中央ディレクトリでの `xl/vbaProject.bin` エントリの存在確認（拡張子のみでの無差別判定はしない） |

対応外（v1の既知の制約。§10残課題）:
- レガシー `.xls`（トップレベル自体がCFBでzip層が無いバイナリ形式）
- パスワード保護されたVBAプロジェクト（クラッキングはスコープ外）

拡張性: `VbaContainerAnalyzer` trait を `domain/vba_analyzer/mod.rs` に定義し、`.docm`/`.pptm` 等コンテナ形式の追加は実装追加のみで対応できる構造とする（CLAUDE.md原則2.2.2、RPAの `RpaAnalyzer` trait と同型）。

#### 8.6.2 抽出方式

1. zipコンテナから `xl/vbaProject.bin` を読み出す
2. `cfb`（Compound File Binary/OLE2パーサ）でストリームを列挙する
3. 各 `/VBA/<ModuleName>` ストリームは「非圧縮のパフォーマンスキャッシュ（p-code）+ MS-OVBA圧縮されたソースコード」の構成のため、ストリーム内でMS-OVBA圧縮コンテナのシグネチャバイト(0x01)候補を探索し復号を試みることでソース開始位置を特定する（`dir`ストリームの厳密なレコード解析による正確なオフセット特定は実サンプル入手後の改善候補、§10残課題）
4. `/PROJECT`ストリーム（プレーンテキスト、MS-OVBA圧縮なし）から `Module=`/`Class=`/`BaseClass=`/`Document=` 行を読み取り、モジュール種別（標準/クラス/フォーム/ドキュメント）を判定する
5. `/VBA/dir`ストリーム（先頭からMS-OVBA圧縮）を復号し、REFERENCENAMEレコード相当のバイト列からベストエフォートで外部参照名を抽出する
6. MS-OVBA圧縮コンテナの解凍アルゴリズム（MS-OVBA仕様§2.4.1）は `domain/vba_analyzer/msovba.rs` に集約し、将来のコンテナ形式追加（.docm/.pptm等）からも共用する（CLAUDE.md原則2.2.1）

#### 8.6.3 共通中間表現

```
VbaComponent {
  componentPath, workbookName,
  modules: [{ name, kind: "standard"|"class"|"form"|"document", loc }],
  externalReferences: [string]
}
```

AI解析には `VbaComponent` に加え、全モジュールのソースを結合した文字列（`combinedSource`）を渡す。

#### 8.6.4 VBA用AI解析

- **専用プロンプト**を `prompts.rs` に追加定義する（`VBA_PROMPT_VERSION` として独立管理。コード/RPA用プロンプト変更でVBAキャッシュが無効化されないようにするため）
- 出力スキーマは §5.1 を流用しつつ、`publicApis` を「公開Sub/Function（Public宣言のプロシージャ）」、`designPatterns` を「マクロの処理パターン（自動化/データ加工/UI操作/外部アプリ連携等）」として解釈させる。この結果は**詳細設計書（VBAマクロコンポーネント単位、§11）**の元データとなる
- キャッシュキーは `component_hash + model + VBA_PROMPT_VERSION`（RPAと同様、ファイル単位キャッシュと独立した名前空間）

#### 8.6.5 設定

`config.scan.vba`（§4）: `enabled` / `extensions` / `maxFileSizeKb`。既定値は性能重視ではなくファイルサイズの実情（Excelワークブックはコードファイルより大きくなりやすい）に合わせて `maxFileSizeKb: 10240` とする。
...
| 解析対象 | ソースコードのみ | ソースコード + **RPA定義ファイル（Power Platform / PAD / UiPath）**（FR-12） + **Excel VBAマクロ（.xlsm/.xlsb）**（FR-13、§8.6） |
| 出力ドキュメント構造 | MD/HTML/JSON形式 × セクション選択による**単一の統合ドキュメント** | **システム仕様書・システム基本設計書・詳細設計書（ファイル/RPA/VBAコンポーネント単位）・ディレクトリ構造（オプトイン）の4成果物**を独立して生成（§11、FR-05/FR-14） |
| フロントエンド基盤 | `src/lib/`（types/constants/tauri/i18n）が `.gitignore` の `lib/`（Pythonテンプレート由来、パス先頭にスラッシュが無く任意深さにマッチ）で誤って無視され、常にコミット対象外だった | `.gitignore` を `/lib/`（ルート限定）に修正し、`src/lib/*` を再構築・コミット対象化 |
...
| 10 | VBAマクロ抽出（§8.6）は実物の`.xlsm`/`.xlsb`サンプル未入手のため、モジュール本文の開始位置特定を`dir`ストリームの厳密なレコード解析ではなくシグネチャバイト探索のフォールバック方式で実装している | 実サンプル入手後に、`dir`ストリーム（PROJECTMODULES/MODULEOFFSET等）の厳密パースへの置き換えを検討し、抽出精度を実測・検証する |
| 11 | VBAマクロ解析はレガシー`.xls`形式・パスワード保護されたVBAプロジェクトをv1では対象外としている | 需要が生じた場合、`.xls`（CFBのみ・zip層なし）対応や、ユーザー入力パスワードでの復号対応を別途検討する |
...
01_要件定義書§4.2（FR-05/FR-14）のとおり、ProjectLensは1回の解析から**独立した4種類のドキュメント**を生成できる。いずれも §7 の `export_document` で形式（Markdown/HTML/JSON）を選択してエクスポートできる。
...
| 詳細設計書 | 開発エンジニア | `FileAnalysisResult`（§5.1）+ 対応する `StaticAnalysisResult`/`RpaComponent`/`VbaComponent` | **ファイル/RPA/VBAコンポーネントごとに1ドキュメント** |
| ディレクトリ構造ドキュメント（FR-14、**オプトイン**） | 全利用者（非エンジニア含む） | `ScanResult.files`（AI/静的解析結果には依存しない） | プロジェクト全体で1ドキュメント |

ディレクトリ構造ドキュメントは他3種と異なり `config.export.defaultDocTypes` の既定値には含めない（エクスポート画面で明示的に選択したときのみ生成される）。
...
├── directory-structure.md    # ディレクトリ構造（オプトイン選択時のみ）
└── detail/                   # 詳細設計書（ファイル/RPA/VBAコンポーネント単位）
    ├── src_foo_bar.ts.md
    ├── src_baz.rs.md
    ├── rpa_flows_承認フロー.robin.md
    └── macros_集計ツール.xlsm.md
```
